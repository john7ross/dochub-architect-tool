...
class PlantUMLParser(BaseParser):
    ...
    def _parse_components(self) -> List[Component]:
        ...
        # Хвост макроса: на настоящих схемах после обязательных аргументов
        # идут именованные — $link="", $tags=, $sprite=. Пустая строка ""
        # тоже встречается, поэтому [^"]* , а не [^"]+
        patterns = {
            'Person': r'Person(?:_Ext)?\s*\(\s*(\w+)\s*,\s*"([^"]*)"(?:\s*,\s*"([^"]*)")?[^)]*\)',
            'System': r'System(?:_Ext)?\s*\(\s*(\w+)\s*,\s*"([^"]*)"(?:\s*,\s*"([^"]*)")?[^)]*\)',
            'Container': r'Container(?:Db)?\s*\(\s*(\w+)\s*,\s*"([^"]*)"(?:\s*,\s*"([^"]*)")?(?:\s*,\s*"([^"]*)")?[^)]*\)',
            'Component': r'Component\s*\(\s*(\w+)\s*,\s*"([^"]*)"(?:\s*,\s*"([^"]*)")?(?:\s*,\s*"([^"]*)")?[^)]*\)',
        }
